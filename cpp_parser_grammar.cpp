    ...
    instance() {
        ...
        angle_bracket_skip_rule = function([](auto& pc) -> bool {
            ...
            if (!pc.is_valid_parse_position() ||
                pc.get_iterator()->get_id() != id_type::LT)
                return false;

            pc.increment_parse_position();
            int angle_level = 1;
            while (angle_level > 0 && pc.is_valid_parse_position()) {
                auto token_id = pc.get_iterator()->get_id();
        ...
        block_skip_rule = function([](auto& pc) -> bool {
            using id_type = typename cpp_lexer_grammar::match_id_type;
            if (!pc.is_valid_parse_position() ||
                pc.get_iterator()->get_id() != id_type::LEFT_BRACE)
                return false;

            pc.increment_parse_position();
            int brace_level = 1;
            while (brace_level > 0 && pc.is_valid_parse_position()) {
                auto id = pc.get_iterator()->get_id();
        ...
        paren_skip_rule = function([](auto& pc) -> bool {
            using id_type = typename cpp_lexer_grammar::match_id_type;
            if (!pc.is_valid_parse_position() ||
                pc.get_iterator()->get_id() != id_type::LEFT_PAREN)
                return false;
...
bool cpp_parser_grammar::parse(parse_context_type& pc) {
    instance inst;
